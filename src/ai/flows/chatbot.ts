...
import { getMedicineDetails } from '@/ai/tools/get-medicine-details';
...
When asked about a specific medicine, you MUST use the getMedicineDetails tool to retrieve the information. After getting the details, you MUST present them to the user in a clear, bulleted, note-like format. The sections should be Uses, Dosage, Side Effects, and Interactions.
...
const chatbotFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const response = await ai.generate({
      ...
      tools: [scheduleMeeting, getMedicineDetails],
